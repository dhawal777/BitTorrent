...
#include "erase1.h"
char buf[BUFSIZE]; 
void tracker2(string a,int PORT,string log,string b,int PORT2);
...
void tracker1(string a,int PORT,string log,string b,int PORT2)
{
    ...
    { 
        ...
        perror("accept");
       thread th5(tracker2,b,PORT2,log,a,PORT); 
       th5.join();  
        exit(EXIT_FAILURE); 
    } 
      int valread;
...
void tracker2(string a,int PORT,string log,string b,int PORT2)
{
    ...
    { 
        perror("accept"); 
        thread th6(tracker1,b,PORT2,log,a,PORT); 
        th6.join();  
        exit(EXIT_FAILURE); 
    } 
      int valread;
    ...
    // cout<<buf[0]<<endl;
     if(buf[0]=='2')//GETTING INFORMED ABOUT ITS TASK
     {thread th7(foo,new_socket,log); 
        th7.join();
     }
    else if(buf[0]=='1')
    {thread th8(share,new_socket,log);
        th8.join();
    }
    else if(buf[0]=='3')
    {
       thread th9(erase,new_socket,log);
       th9.join();
    }
    else if(buf[0]=='4')
    {
        thread th10(rm_torrent,new_socket,log);
         th10.join();
    }
...
{ 
    ...
    geek1 >> x1; 
     ...
     thread th0(tracker1,ip1,x,s_seed,ip2,x1); 
      th0.join();  
